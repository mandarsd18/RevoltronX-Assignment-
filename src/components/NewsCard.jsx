const NewsCard = ({ name, data }) => {
  return (
    <>
    {/* Top Healine are Latest news  */}
      <div className="flex items-center mb-2">
        <h2 className="text-lg font-bold sm:text-xl md:text-2xl" id={name}>
          {name}
        </h2>
      </div>

      <div className="flex flex-no-wrap overflow-y-hidden overflow-x-scroll  scrollbar-hide items-start mb-3 md:mb-8">
        {data?.map((item) =>
          // Where news in removed from the Api
          item.source.name == "[Removed]" ? (
            ""
          ) : (
          // Each card in news Sections
